...
export default function AgendamentoItem(props: AgendamentoItemProps) {
  ...
  function somarTotalServicos() {
    return props.agendamento.servicos.reduce((acc, service) => acc + service.price, 0)
  }

  function renderizarServicos() {
    return props.agendamento.servicos.reduce((acc, service, index) => {
      return `${acc}${index + 1}. ${service.name}${index < props.agendamento.servicos.length - 1 ? ", " : ""}`
    }, "")
  }
  ...
  return (
    <View style={{ ...styles.cartao, borderColor: cor }}>
      <Text style={{ ...styles.nomeProfissional }}>
        {props.agendamento.profissional.name
          ? props.agendamento.profissional.name
          : "Não informado"}
      </Text>
